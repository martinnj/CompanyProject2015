...
# Since random forests only allow for features with 32 levels at max, we need to
# change FamiliyID to something smaller. Let's expand the 'small' group and see
# if that works.
...
# And now to make a forrest, first, seed the random generator
set.seed(1337)
...
# We can get some test info from the new tree to see how variables affect the
# model we created.
varImpPlot(fit)
...
write.csv(submit, file="titanic_06_prediction.csv", row.names=F)
